...
Speed of Light					& \qquad $c$			& \qquad 299792458 $\si{\meter\per\second}$\\
Electron charge					& \qquad $q$ 			& \qquad 1.6021766208 $\cdot10^{-19}~\si{\coulomb}$ \\
Electron mass					& \qquad $m_e$ 			& \qquad 9.10938356 $\cdot 10^{-31}~\si{\kilogram}$ \\
...
Electromagnetic Field Tensor		& \qquad $F^{\mu}_{\nu}$\\
...
\noalign{\vskip 5mm}
Poynting Vector					& \qquad $\vec{S}$			&\qquad $[\si{\newton\per\meter\per\second}]$\\
Position						& \qquad $\vec{r}$			& \qquad $[\si{\meter}]$\\
Velocity						& \qquad $\vec{v}$			& \qquad $[\si{\meter\per\second}]$\\
Normalized Velocity				& \qquad $\vec{\beta} = \frac{\vec{v}}{c}$ 		& \qquad $[\si{}]$ \\
Normalized Acceleration			& \qquad $\dot{\vec{\beta}}$ 	& \qquad $[\si{\per\second}]$ \\
Wavelength					& \qquad $\lambda$			& \qquad $[\si{\meter}]$\\
Wavevector					& \qquad $\vec{k}$			& \qquad $[\si{\per\meter}]$\\
%Beschleunigung				& \qquad $\vec{a}$ 			& \qquad $[\si{\meter\per\square\second}]$ \\ 
%Impuls						& \qquad $\vec{p}$			& \qquad $[\si{\kilogram\meter\per\second}]$ \\
Force						& \qquad $\vec{F}$ 			& \qquad $[\si{\newton}]$ \\
Mass						& \qquad $m$ 				& \qquad $[\si{\kilogram}]$ \\
Energy						& \qquad $E$ 				& \qquad $[\si{\joule}]$ \\
Power						& \qquad $W$ 				& \qquad $[\si{\watt}]$ \\
...
Time 						& \qquad t 				& \qquad $[\si{\second}]$\\
discretized Time				& \qquad $h$				& \qquad $[\si{\second}]$\\
Frequenzy					& \qquad $\omega$			& \qquad $[\si{\per\second}]$\\
