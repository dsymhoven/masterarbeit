...
\eeq
\noindent
This and more examples as well as a more detailed explanation can be found in \cite{Herzing}. All points along the boarder between Near - and Farfield need to be adjusted accordingly. \\
\newline
...
\begin{figure}[H]
	...
	\caption[]{Section of a grid with three boxes, two of which are Nearfield boxes (NF) and one being a Farfield box (FF). The colored planes represent those grid points, where adjustments are necessary, as explained above. One $\vec{E}$ - Field push consists of four steps. or equivalently four subroutines in Code: 1. \texttt{pushEFieldInsideBoxes}, 2. \texttt{setHFieldOnBoarders}, 3. \texttt{adjustHFields}, 4 \texttt{pushEFieldAtBorders}. For reasons of clarity we left out the grid points within the boxes. }
...
\item \texttt{setHFieldOnBoarders()}
\item \texttt{adjustHField()}
\item \texttt{pushEFieldOnBoarders()}
\end{enumerate}
...
\subsection{setHFieldOnBoarders()}
...
\begin{figure}[H]
\subfigure[t = 7]{\includegraphics[width=0.33\textwidth]{Pictures/nearFieldOneParticle.png}}\hfill
\subfigure[t = 9]{\includegraphics[width=0.33\textwidth]{Pictures/nearFieldTwoParticles.png}}\hfill
\subfigure[t = 11]{\includegraphics[width=0.33\textwidth]{Pictures/nearFieldTwoParticles.png}}
\caption[]{Simulation of an electron with $\gamma = 1,1$ in a constant $B_z = 1$ field. The colorbar shows the value of $|\vec{E}|^2$. $\vec{E}$ and $\vec{B}$ are in units of $4\pi\epsilon_0m_e^2c^4/e^3 = 1.77 \cdot 10^{20} \si{\volt\per\meter}$, length in units of $e^2 / (4\pi\epsilon_0m_ec^2) = 2.82 \cdot 10^{-15}\si{\meter}$ and times in units of $e^2 / (4\pi\epsilon_0m_ec^3 = 9.40 \cdot 10^{-24}\si{\second}$}	
...
In order to push the particle properly not just the external fields but also the field contributions from other particles propagated into the Nearfield region need to be taken into account. 
