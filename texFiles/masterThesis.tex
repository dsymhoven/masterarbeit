...
\eeq
holds. The idea is to calculate each point along the trajectory $x^{\mu}_i = x^{\mu}(\tau_i)$ iteratively, starting from the initial values $x^{\mu}_0$ and $u^{\mu}_0$.
...
\end{description}
\end{env_definition}
Anschaulich ist der Procedural Error die Differenz zwischen der exakten Lösung $y(\tau+h)$ und dem Ergebnis, das man durch das one-step scheme erhält, wenn man von der exakten Lösung zum früheren Zeitpunkt $y(\tau)$ startet. Abbildung \ref{fig: Procedural Error} veranschaulicht die Situation.
\begin{figure}[H]
	\centering
		\includegraphics[width=0.50\textwidth]{Pictures/Verfahrensfehler.png} %Bild 1
	\caption[Procedural Error]{Veranschaulichung des Procedural Errors eines one-step schemes.~ \cite{NumerikSkript}}
	\label{fig: Procedural Error}
\end{figure}
\noindent
Wir wollen nun die Definitionen anwenden, um die Order of Consistency  des Euler-Verfahrens zu bestimmen. \\
...
Nach Definition \ref{def: Procedural Error} gilt für unser System
\beq
y = u^{\mu}. \nn \\
\eeq
Wir erhalten
...
\eeq
$\mathcal{D}(u^{\mu})$ bezeichnet dabei den Definitionsbereich von $u^{\mu}$. Das Euler-Verfahren hat also Order of Consistency  eins.

\subsection{Leap-Frog-Verfahren}
Ein deutlich besseres Verfahren ist das \textit{Leap-Frog}-Verfahren, das eine Order of Consistency  von zwei hat, wie man leicht nachrechnen kann. \\
