...
\pagenumbering{roman}
  ...
  \LMUTitle
      ...
      {Physikfakultät}                         % Name der Fakultaet
...
% MISSING: More text about why N^2 and what the basic idea behind hybrid fields is
...
\vec{\nabla} \times \vec{B} &=  \frac{\del \vec{E}}{\del t}
\eal
\eeq
\subsection{The Yee-Scheme}
To push the fields on the grid we use the \textit{Yee- Scheme} introduced by \textit{Kane Yee} in 1966 \cite{Yee66}.\\
Figure \ref{fig: YeeBox} illustrates a so called Yee-Box and how to solve the curl equations of \eqref{eq: maxwellEq}.
\begin{figure}[H]
	\centering
		\includegraphics[width=0.70\textwidth]{Pictures/YeeBox.png} 
	\caption[Yee-Box]{An illustration of a Yee-Box which is used to solve the curl Maxwell Equations \cite{Taflove}.}
	\label{fig: YeeBox}
\end{figure}
