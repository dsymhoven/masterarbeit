...
\newline
Christian Herzing developed in his PhD-thesis an hybrid field approach to improve PIC even further \cite{Herzing}. By introducing near and far fields he could reduce the numeric complexity to almost $\mathcal{O}(N)$. His framework is written in \texttt{C++}. Unfortunately this is not compatible with the \textit{Plasma Simulation Code} (PSC), which requires \texttt{C} code input. PSC is a massively parallel code framework for the simulation of plasma, laser-plasma, high field physics in plasma and vacuum, and Molecular Dynamics, developed by members of the Computational \& Plasma physics chair of Prof. Dr. Hartmut Ruhl. So, the goal of this work is to translate the framework into \texttt{C} and to build a solid and well documented code base others can build upon. To show that the framework is applicable, a real world use case, that is the scattering of an electron in a electromagnetic wave, is simulated. \\
\newline
The thesis is structured in three parts. The first part deals with the fundamentals. One key aspect of this thesis are the Liénard-Wiechert fields, which will be derived analytically. Starting with the derivation of the inhomogeneous wave equation using potential equations, we will afterwards solve the inhomogeneous wave equation using greens function approach, which will lead us to the Liénard-Wiechert fields.\\
...
A summary and outlook is given in the third part. The presented framework in \cite{Herzing} was entirely rewritten in \texttt{C} and furthermore extended with new features, like a reverse Boris Push, trilinear interpolation of fields, effective storage of precalculated fields and UPML. \\
...
\newline
In order to find $\frac{\omega_L}{\omega}$ we start with the laser amplitude $a_0$, which is usually of the order
\beq
...
We show that $\vec{E}' = \vec{E}$ and $\vec{B}' = \vec{B}$.
\beq
\bal
