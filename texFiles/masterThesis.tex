...
\newline
Because of its simplicity PIC is the standard simulation technique for plasma physics today \cite{Dawson, Birdsall, Cohen}. PIC makes use of the concept of quasi-particles and a Maxwell grid to calculate electric currents at discrete grid points. The Maxwell grid is used to solve discrete Maxwell equations. This way an interaction with the quasi-particles and the fields on the Maxwell grid is possible. Whereas first simulations were limited to one dimensional problems due to a lack of computational power, nowadays three dimensional simulations are feasible \cite{Fonseca, Pukhov, Bonitz}. For instance, a MD simulation with $10^{8}$ particles for 100000 time steps requires about 3 years on a modern 100 TFLOP/s super computer. The same simulation using PIC only requires three seconds \cite{Ren}.\\
\newline
Christian Herzing developed in his PhD-thesis an hybrid numerical technique to improve MD simulations such that their numerical load compares to the one of PIC in many situations \cite{Herzing}. By introducing the concept of near and far fields Herzing has been able to reduce the numeric complexity of MD simulations with radiation to almost $\mathcal{O}(N)$. His framework is written in \texttt{C++}. Unfortunately this is not compatible with the \textit{Plasma Simulation Code} (PSC), which requires \texttt{C} code input. PSC is a massively parallel code framework for the simulation of plasma, laser-plasma, high field physics in plasma and vacuum, and MD, developed by members of the Computational \& Plasma physics chair of Prof. Dr. Hartmut Ruhl. The goal of the present work is to translate the framework of Herzing into \texttt{C} and to create a solid and well documented code base others can build upon. To show that the framework is applicable to a real world case the scattering of an electron in a electromagnetic wave is simulated. \\
\newline
...
\eeq
Using the relationship between $\omega_L$ and $\omega$ one can find 
\beq
\bal
...
\eal
\eeq
The same holds for $B_{0L}$. Finally, the grid resolution $dx$ needs to be determined and therefore $dt$. In order to resolve the fields properly on the grid, the time resolution needs to be so small that 
\beq
...
\end{lstlisting}
For the external laser pulse a cosine function is modulated with a sin squared function in x direction and a gaussian in y direction. Figure \ref{fig: external Pulse} shows the external pulse. 
