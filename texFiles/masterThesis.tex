...
The Maxwell's Curl Equations then read
\beq
\bal
\vec{\nabla} \times \vec{E} &= -j\omega\mu_0\left[\mu_r\right]\vec{H}, \\
\vec{\nabla} \times \vec{H} &= j\omega\epsilon_0\left[\epsilon_r\right]\vec{E}.
\eal
\eeq
When incorporating loss into Maxwell Equations we have two possibilities. We can either choose a complex permittivity, which makes it easier to work in frequency domain or we can choose to have reel permittivity $\epsilon_r$ and reel conductivity $\sigma$. We will use the latter, since we work in a time domain model. We end up with
\beq
\bal
\begin{bmatrix}
...
In the case of a wave with arbitrary polarization and angle of incidence, as illustrated in figure \ref{fig: Fresnel}, we can use \textit{Fresnel} equations. For doubly anisotropic media, i.e 
$ \left[\epsilon_r\right] = \left[\mu_r\right] = 
\begin{bmatrix}
a & 0 & 0  \\
0 & b &0  \\
0 & 0 & c 
\end{bmatrix}$ with $a, b, c \in \mathbb{R}$ they read 
\begin{figure}[t]
	\centering
...
\eeq
From the original Fresnel equations we know that the reflection is a consequence of a change in impedance and also critically angle dependent.  
